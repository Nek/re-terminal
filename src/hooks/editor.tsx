...
import { SharedState, useSharedState } from "@nekocode/use-shared-state";
import { useMemo } from "react";

const editorInputSharedState = new SharedState("");
const processCurrentLineSharedState = new SharedState(false);
...
export const useEditorCommands = (): {
  ...
  const editorInputState = useMemo(() => editorInputSharedState, []);
  const processCurrentLineState = useMemo(
    () => processCurrentLineSharedState,
    []
  );

  const [editorInput, setEditorInput] = useSharedState(editorInputState);

  const [processCurrentLine, setProcessCurrentLine] = useSharedState(
    processCurrentLineState
  );
...
export const useEditorInput = (
  ...
  editorInput: string,
  setEditorInput: (s: string) => void,
  setProcessCurrentLine: (flag: boolean) => void,
  caretPosition: number,
  setCaretPosition: (position: number) => void,
  setBeforeCaretText: (s: string) => void,
  setAfterCaretText: (s: string) => void,
  enableInput: boolean
) => {
